import json
import matplotlib.pyplot as plt
from matplotlib import rcParams, colors
import numpy as np
import os 
import math
from brokenaxes import brokenaxes
from mpl_toolkits.axes_grid1.inset_locator import inset_axes




########################### PLOT SETTINGS ###########################
## The following is taken from: https://github.com/quantum-journal/quantum-journal/tree/v5.0
_widths = {
    'onecolumn': {
        'a4paper' : 5.93,  # a4paper columnwidth = 426.79135 pt = 5.93 in
        'letterpaper' : 6.16  # letterpaper columnwidth = 443.57848 pt = 6.16 in
    },
    'twocolumn': {
        'a4paper' : 3.22,  # a4paper columnwidth = 231.84843 pt = 3.22 in
        'letterpaper' : 3.34  # letterpaper columnwidth = 240.24199 pt = 3.34 in
    }
}
_wide_widths = {
    'onecolumn': {
        'a4paper' : 5.93,  # a4paper wide columnwidth = 426.79135 pt = 5.93 in
        'letterpaper' : 6.16  # letterpaper wide columnwidth = 443.57848 pt = 6.16 in
    },    
    'twocolumn': {
        'a4paper' : 6.72,  # a4paper wide linewidth = 483.69687 pt = 6.72 in
        'letterpaper' : 6.95  # letterpaper wide linewidth = 500.48400 pt = 6.95 in
    }
}
_fontsizes = {
    10 : {
        'tiny' : 5,
        'scriptsize' : 7,
        'footnotesize' : 8, 
        'small' : 9, 
        'normalsize' : 10,
        'large' : 12, 
        'Large' : 14, 
        'LARGE' : 17,
        'huge' : 20,
        'Huge' : 25
    },
    11 : {
        'tiny' : 6,
        'scriptsize' : 8,
        'footnotesize' : 9, 
        'small' : 10, 
        'normalsize' : 11,
        'large' : 12, 
        'Large' : 14, 
        'LARGE' : 17,
        'huge' :  20,
        'Huge' :  25
    },
    12 : {
        'tiny' : 6,
        'scriptsize' : 8,
        'footnotesize' : 10, 
        'small' : 11, 
        'normalsize' : 12,
        'large' : 14, 
        'Large' : 17, 
        'LARGE' : 20,
        'huge' :  25,
        'Huge' :  25
    }
}
_width         = 1
_wide_width    = 1
_quantumviolet = '#53257F'
_quantumgray   = '#555555'
def global_setup(columns='twocolumn', paper='a4paper', fontsize=10):
    plt.rcdefaults()
    plt.style.use(['seaborn-v0_8-white', 'quantum-plots.mplstyle']) 
    global _width 
    _width = _widths[columns][paper]
    global _wide_width 
    _wide_width = _wide_widths[columns][paper]
    ## Use the default fontsize scaling of LaTeX
    global _fontsizes
    fontsizes = _fontsizes[fontsize]
    plt.rcParams['axes.labelsize']  = fontsizes['normalsize']
    plt.rcParams['axes.titlesize']  = fontsizes['normalsize']
    plt.rcParams['xtick.labelsize'] = fontsizes['footnotesize']
    plt.rcParams['ytick.labelsize'] = fontsizes['footnotesize']
    plt.rcParams['font.size']       = fontsizes['small']
    plt.rcParams['legend.fontsize'] = fontsizes['footnotesize']
    rcParams['text.usetex'] = True
    plt.rcParams['font.family'] = 'serif'
    return {
            'fontsizes' : fontsizes,
            'colors' : {
                'quantumviolet' : _quantumviolet,
                'quantumgray' : _quantumgray
            }
        }

# Sets up the plot with the fitting arguments so that the font sizes of the plot
# and the font sizes of the document are well aligned
#
#     aspect_ratio : float
#         the aspect ratio (width/height) of your plot
#         defaults to the golden ratio
#
#     width_ratio : float in [0, 1]
#         the width of your plot when you insert it into the document, e.g.
#         .8 of the regular width
#         defaults to 1.0
#
#     wide : bool 
#         indicates if the figures spans two columns in twocolumn mode, i.e.
#         when the figure* environment is used, has no effect in onecolumn mode 
#         defaults to False
#
#     (returns) : matplotlib figure object
#         the initialized figure object
def plot_setup(aspect_ratio=1/1.62, width_ratio=1., wide=False): 
    width = (_wide_width if wide else _width) * width_ratio
    height = width * aspect_ratio
    return plt.figure(figsize=(width,height), dpi=200, facecolor='white')

def plot_with_inset_setup(aspect_ratio=1/1.62, width_ratio=1., wide=False):
    width = (_wide_width if wide else _width) * width_ratio
    height = width * aspect_ratio
    fig, ax = plt.subplots(figsize=(width,height), dpi=200, facecolor='white')
    # # these are in unitless percentages of the figure size. (0,0 is bottom left):
    # left, bottom, width, height = [0.64, 0.48, 0.3, 0.43] 
    # ax_inset = fig.add_axes([left, bottom, width, height])
    ax_inset = inset_axes(ax, width="60%", height="60%", loc='upper right')
    return ax, ax_inset

global_setup(columns='twocolumn', paper='a4paper', fontsize=10)


## Define the color and marker settings 
def lighter(color, percent):
    """ Assume the "color" variable is rgb between (0,0,0) and (1,1,1). Function based 
    on https://stackoverflow.com/questions/28015400/how-to-fade-color. """
    color = np.array(color)
    white = colors.to_rgb('white')
    vector = white-color
    return color + vector * percent

def convert_rgb_255_to_1_scale(color: tuple):
    return tuple(np.array(color)/255)

blue = convert_rgb_255_to_1_scale((0,119,187))
green = convert_rgb_255_to_1_scale((0,153,68))

# light_blue = lighter(colors.to_rgb(blue), 0.7)
# light_green = lighter(colors.to_rgb(green), 0.7)

colours = [blue, green]
# light_colours = [light_blue, light_green]













## Define global variables
best_energy_N30_n009 = 4.208  # [K]
part_of_optimization = '3'  # '1', '2' or '3'

N = 30
d = 2
density = 0.09 # 1/Ang^d
phase = 'solid'
ns = 1024  # number of (optimized) samples to consider 
alphaHe4 = 1.37643  
alphaHe3 = 1.66185  
alpha = alphaHe4  # factor to reintroduce units (of kelvin)
bt = 4
ld = 8
nf = 8
mlp_layers = 1
extra_string = '_no_h-fixed_n_samples=72-32' #'_more-samples' # '_2hl_3hv_n_samples=72-32' 

current_dir = os.getcwd()
folderpath = current_dir + '/data_log/' 




# ########################################################################
# #             Plot all three optimization parts together               #
# ########################################################################

# for part_of_optimization in ['1', '2', '3']:
#     datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

#     ## Unpack the data
#     data_log = json.load(open(folderpath + datafilename))
#     iters = np.array(data_log['Energy']['iters'])
#     tot_energies = np.array(data_log['Energy']['Mean'])

#     ## Plot the optimization process
#     plt.plot(iters, alpha * tot_energies / N)
#     plt.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)

# plt.xlabel('Iterations')
# plt.ylabel('E/N [K] ')
# # outfilename = f'energy_vs_iters_he4_N={N}_d=2_density={density:.3f}_bt=4_ld=8_nf=8_hd=1_branch={phase}.pdf'
# # plt.savefig(outfilename, dpi=400, bbox_inches='tight')
# plt.show()






# #####################################################################
# #               Optimization part 1: pre-training                   #
# #####################################################################
# part_of_optimization = '1'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])


# # fig, ax = plt.subplots()
# ax, ax_inset = plot_with_inset_setup()
# ax.plot(iters, alpha * tot_energies / N)
# ax.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)

# # Create inset of the zoomed region
# # ax_inset = inset_axes(ax, width="50%", height="50%", loc='upper right')
# ax_inset.plot(iters, alpha * tot_energies / N)
# ax_inset.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
# ax_inset.set_xlim(10, 41)  # Adjust as needed
# ax_inset.set_ylim(4.1, 8.5)  # Adjust as needed
# # ax_inset.set_xticklabels([])
# # ax_inset.set_yticklabels([])

# ax.set_xlabel('Iterations')
# ax.set_ylabel('E/N [K]')

# plt.tight_layout()
# plt.show()

# #####################################################################
# #                 Optimization part 2: training                     #
# #####################################################################

# plot_setup()
# part_of_optimization = '2'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])

# # Create the broken axes plot
# time_segments = ((0,500),(2000,2500),(24500,25000))
# energy_segments = ((4.15,4.4),(5.8,6))
# bax = brokenaxes(xlims=time_segments, ylims=energy_segments, hspace=0.2, wspace=0.5)

# bax.plot(iters, alpha * tot_energies / N)
# bax.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)

# # Add labels and legend
# bax.set_xlabel('Iterations')
# bax.set_ylabel('E/N [K]')

# # plt.tight_layout()
# plt.show()

# #####################################################################
# #                Optimization part 1: fine-tuning                   #
# #####################################################################

# plot_setup()
# part_of_optimization = '3'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])

# plt.plot(iters, alpha * tot_energies / N)
# plt.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)

# plt.xlabel('Iterations')
# plt.ylabel('E/N [K]')

# plt.tight_layout()
# plt.show()










import matplotlib.pyplot as plt
import numpy as np
import json
from brokenaxes import brokenaxes
from mpl_toolkits.axes_grid1.inset_locator import inset_axes
import matplotlib.gridspec as gridspec



def plot_setup(aspect_ratio=1/1.62, width_ratio=1., wide=False): 
    width = (_wide_width if wide else _width) * width_ratio
    height = width * aspect_ratio
    width *= 3
    return plt.figure(figsize=(width,height), dpi=200, facecolor='white')

fig = plot_setup()
# fig = plt.figure(figsize=(12,4))
gs = fig.add_gridspec(1, 3, width_ratios=[1, 1.1, 1])

#####################################################################
#               Optimization part 1: pre-training                   #
#####################################################################

ax0 = fig.add_subplot(gs[0])  

part_of_optimization = '1'
datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

## Unpack the data
data_log = json.load(open(folderpath + datafilename))
iters = np.array(data_log['Energy']['iters'])
tot_energies = np.array(data_log['Energy']['Mean'])

ax0.plot(iters, alpha * tot_energies / N, color=colours[0])
ax0.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
ax0.set_title('Pre-training')
ax0.set_xlabel('Iterations')
ax0.set_ylabel('$E/N$ [K]')
ax0.text(0.98, 0.15, '4 min', color=colours[1], transform=ax0.transAxes, verticalalignment='bottom', horizontalalignment='right')
ax0.axvline(x=40, ymin=0, ymax=17/ax0.get_ylim()[1], color=colours[1], linestyle='solid')

# Create inset of the zoomed region
ax_inset = inset_axes(ax0, width="60%", height="60%", loc='upper right')
ax_inset.plot(iters, alpha * tot_energies / N)
ax_inset.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
ax_inset.set_xlim(10, 41)  
ax_inset.set_ylim(4.1, 8.5) 

#####################################################################
#                 Optimization part 2: training                     #
#####################################################################

 
part_of_optimization = '2'
datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

## Unpack the data
data_log = json.load(open(folderpath + datafilename))
iters = np.array(data_log['Energy']['iters'])
tot_energies = np.array(data_log['Energy']['Mean'])

# Create broken axes
time_segments = ((0,500),(2000,2500),(24500,25000))
energy_segments = ((4.15,4.4),(5.8,6))
bax = brokenaxes(
    xlims=time_segments,
    ylims=energy_segments,
    hspace=0.5,
    wspace=0.7, 
    subplot_spec=gs[1],
    d=0.005,  # size of the tilted (breaking-axis) bars
)

bax.plot(iters, alpha * tot_energies / N)
bax.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
bax.set_title('Training')

# Manually add xlabel (because I cannot add the axis "ax1 = fig.add_subplot(gs[1])", else it breaks the formatting for some reason...)
fig.text(0.5, -0.01, 'Iterations', ha='center', va='center')



## To artificially add top and right borders to the bax plot.
## It's important to add this subplot after the bax subplot to make 
## the borders appear on top of the brokenaxes plot.
ax2 = fig.add_subplot(gs[1])
# ax2.set_xticklabels([])
# ax2.set_yticklabels([])
ax2.set_xticks([])
ax2.set_yticks([])
ax2.spines['left'].set_visible(False)
ax2.spines['bottom'].set_visible(False)
ax2.patch.set_alpha(0)  # Make the interior of the plot transparent


ax2.text(0.12, 0.52, '15 min', color=colours[1], transform=ax2.transAxes,)
ax2.axvline(x=0.23, ymin=0, ymax=0.5, color=colours[1], linestyle='solid')

ax2.text(0.72, 0.52, '14 hours', color=colours[1], transform=ax2.transAxes,)
ax2.axvline(x=0.94, ymin=0, ymax=0.5, color=colours[1], linestyle='solid')



#####################################################################
#                Optimization part 3: fine-tuning                   #
#####################################################################

ax3 = fig.add_subplot(gs[2]) 

part_of_optimization = '3'
datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

## Unpack the data
data_log = json.load(open(folderpath + datafilename))
iters = np.array(data_log['Energy']['iters'])
tot_energies = np.array(data_log['Energy']['Mean'])

ax3.plot(iters, alpha * tot_energies / N)
ax3.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
ax3.set_title('Fine-tuning')
ax3.set_xlabel('Iterations')

ax3.text(0.55, 0.93, '$\sim$ a few days', color=colours[1], transform=ax3.transAxes)

# plt.tight_layout()  # !! cannot use because it adjusts the spacing between subplots, which can interfere with brokenaxes and misalign the breaking bars !!
output_filename = f'optimization_steps_energy_vs_iters_he4_N=30_d=2_bt=4_ld=8_nf=8_hd=1.pdf'
plt.savefig(output_filename, dpi=300, bbox_inches='tight')
plt.show()











##       DOESNT WORK:      ##


# fig, axs = plt.subplots(1, 3, figsize=(11, 4))

# #####################################################################
# #               Optimization part 1: pre-training                   #
# #####################################################################

# part_of_optimization = '1'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])

# axs[0].plot(iters, alpha * tot_energies / N)
# axs[0].axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
# axs[0].set_title('Optimization part 1: pre-training')


# # Create inset of the zoomed region
# ax_inset = inset_axes(axs[0], width="50%", height="50%", loc='upper right')
# ax_inset.plot(iters, alpha * tot_energies / N)
# ax_inset.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
# ax_inset.set_xlim(10, 41)  # Adjust as needed
# ax_inset.set_ylim(4.1, 8.5)  # Adjust as needed


# axs[0].set_xlabel('Iterations')
# axs[0].set_ylabel('$E/N$ [K]')

# #####################################################################
# #                 Optimization part 2: training                     #
# #####################################################################

# part_of_optimization = '2'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])

# # Create the broken axes plot
# time_segments = ((0,500),(2000,2500),(24500,25000))
# energy_segments = ((4.15,4.4),(5.8,6))
# # bax = brokenaxes(xlims=time_segments, ylims=energy_segments, hspace=0.2, wspace=0.2, subplot_spec=axs[1].get_subplotspec())
# bax = brokenaxes(xlims=time_segments, ylims=energy_segments, hspace=0.2, wspace=0.2, fig=fig, subplot_spec=fig.add_gridspec(1,3)[1])

# bax.plot(iters, alpha * tot_energies / N)
# bax.axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)

# # Add labels and legend
# bax.set_title('Optimization part 2: training')
# bax.set_xlabel('Iterations')

# #####################################################################
# #                Optimization part 3: fine-tuning                   #
# #####################################################################

# part_of_optimization = '3'
# datafilename = f'he4_N=30_d=2_density=0.090_bt=4_ld=8_nf=8_hd=1_branch=solid_p{part_of_optimization}.log'

# ## Unpack the data
# data_log = json.load(open(folderpath + datafilename))
# iters = np.array(data_log['Energy']['iters'])
# tot_energies = np.array(data_log['Energy']['Mean'])

# axs[2].plot(iters, alpha * tot_energies / N)
# axs[2].axhline(best_energy_N30_n009, color='k', linestyle='--', zorder=10)
# axs[2].set_title('Optimization part 3: fine-tuning')
# axs[2].set_xlabel('Iterations')

# plt.tight_layout()
# plt.show()
