...
class MLPFactory:
    ...
    # def build_mlp(self):
    #     mlp = MLP(self.embedding_dim, (self.intermediate_dim,) * self.mlp_layers)
    #     # layer_norm = nn.LayerNorm(bias_init=zeros, scale_init=zeros)
    #     layer_norm = nn.RMSNorm(scale_init=zeros)
...
class MPNNLayer(nn.Module):
    ...
    def __call__(self, yi, Yij, Iij, i):
        ...
        # yi = mlp3(jnp.concatenate((yi, jnp.mean(Mij, axis=-2)), axis=-1))
