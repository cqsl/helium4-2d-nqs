...
def get_local_kernel_arguments(vstate: MCState, Ô: ContinuousOperator):  # noqa: F811
    ...
    Lx = vstate.variables["params"]["Lx_param"]
    Ly = vstate.variables["params"]["Ly_param"]
    theta = vstate.variables["params"]["theta_param"]

    args = tuple(jnp.concatenate((Lx, Ly, theta)).tolist())

    if type(Ô) == SumOperator:  # if Ô is the Hamiltonian (assume 1 kinetic term and 1 potential term)
        Ô._operators[0].arguments = args
        Ô._operators[1].arguments = args

    elif type(Ô) == KineticEnergy or type(Ô) == PotentialEnergy:
        Ô.arguments = args
...
def _expect(
    ...
    L_σ, _ = local_value_kernel(logpsi, parameters, σ, local_value_args)
