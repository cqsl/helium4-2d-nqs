...
class SumOperator(ContinuousOperator):
    ...
    def _expect_kernel(
        ...
        O_tot, dO_tot_dLx, dO_tot_dLy, dO_tot_dtheta = [], [], [], []
        for i, op in enumerate(data.ops):
            coeff = data.coeffs[i]
            op_data = data.op_data[i]
            O, (dO_dLx, dO_dLy, dO_dtheta) = op._expect_kernel(logpsi, params, x, op_data)
            O *= coeff  # coeff is a scalar while O is a 1D array
            dO_dLx *= coeff
            dO_dLy *= coeff 
            dO_dtheta *= coeff 
            O_tot.append(O)
            dO_tot_dLx.append(dO_dLx)
            dO_tot_dLy.append(dO_dLy)
            dO_tot_dtheta.append(dO_dtheta)

        return sum(O_tot), (sum(dO_tot_dLx), sum(dO_tot_dLy), sum(dO_tot_dtheta)) # (n_samples_per_rank,)
